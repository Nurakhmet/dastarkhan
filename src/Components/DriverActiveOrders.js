...
function AdminOrders({newRestaurantAddedId}) {
    ...
    const [order, setOrder] = useState([{id:0, orderName:"",price:"",isActive:true, users:"",dishes:""}]);
    const [data, setData] = useState([]);
    const [change,setChange] = useState(0);

    async function getAllOrders(){
        const bearer = "Bearer "+ localStorage.getItem('jwtToken');
        const response = await fetch("http://localhost:8000/api/allOrdersTrue", {
            method: "GET",
            headers: {
    ...
    async function editOrder(data){
        ...
        const response = await fetch("http://localhost:8000/api/changeOrderStatus/"+data.id, {
    ...
    //
    const rest = data.map(user=>(
        <tr>
            <td>{user.id}</td>
            <td>{user.orderName}</td>
            <td>{user.price}</td>
            {user.active === true ? <td>Active</td> : <td>Not Active</td>}
            ...
            <td>
                <a className="waves-effect waves-light btn" style = {{backgroundColor:"#2D4059", color: "#FFD460"}}
                  onClick={() => editOrder(user)}
                   >Get</a>
            </td>
        </tr>
    ));
